...
/**
 * Config migrations: each is a pure function over the parsed tree, and the runner refuses config
 * newer than the build, backs up the whole tree to `state/backups/` first, and writes all-or-nothing.
 */
...
async function readTree(configDir: string): Promise<Map<string, Document>> {
  ...
  const walk = async (dir: string, prefix: string): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      // overrides.local.json is per-machine and hand-maintained; never rewrite it.
...
export async function migrateConfig(options: {
  ...
  // Whole-tree backup before anything is written.
  ...
  // Written only after every migration succeeded, so a failure leaves the tree untouched.
